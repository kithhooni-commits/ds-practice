...
| 눈·입 구멍 메우기 | 852 → **904 삼각형** | 테셀레이션이 눈꺼풀 둘레만 덮는다 |
| 사진 배경 오염 | 79.7% → **0%** | 렌더 픽셀로 측정 |
...
| 검증 | 하니스 **13종** | 로직 7 + 브라우저 6 (렌더 픽셀 검사 2종) |
...
- **뒤통수는 지어내지 않기로 했다.** Face Mesh는 얼굴 **앞면만** 담는 모델이다.
  얼굴 테두리(FACE_OVAL 36점)에서 두개골을 만들어 붙여 봤고 비율까지 맞췄지만
  (깊이/폭 0.70 → 1.04), 사람마다 테두리 모양·깊이가 크게 달라 실제 얼굴에서는
  **뒤로 뾰족한 혹**이 생겼다. 상수 몇 개로 맞출 수 있는 문제가 아니었다.
  지금은 얼굴을 등록하면 **아바타 머리를 통째로 숨기고 얼굴만 남긴다** —
  정면에서 하는 게임이라 성립하는 타협이다.
