...
| :--- | :---: | :---: | :---: |
| **F1-Score (종합)** | **0.3863** | $\ge 0.850$ | 🟡 REVIEW |
| **Precision (정밀도)** | **0.2881** (17/59) | $\ge 0.800$ | 🟡 REVIEW |
| **Recall (재현율)** | **0.5862** (17/29) | $\ge 0.850$ | 🟡 REVIEW |
| **종류 분류 정확도 (Kind Acc)** | **23.5%** | $\ge 90.0\%$ | 🟡 REVIEW |
| **비동작/풋워크 오검출 (Non-Action FP)** | **11회** | $\le 3회$ | 🔴 과검출 발생 |
| **평균 타격 지연 시간 (Timing MAE)** | **111.9 ms** | $\le 50.0 ms$ | 🟡 REVIEW |
...
| **⏸ 숨고르기 (Rest 1)** | `18~23s` | 휴식/준비 | **1회** | ⚠️ 1회 오검출 |
| **3. 훅 펀치 (Hook)** | `23~35s` | 동작 | **17회** | 🟢 정상 |
| **⏸ 숨고르기 (Rest 2)** | `35~40s` | 휴식/준비 | **0회** | 🟢 정상 |
| **4. 어퍼컷 (Uppercut)** | `40~52s` | 동작 | **15회** | 🟢 정상 |
| **⏸ 숨고르기 (Rest 3)** | `52~57s` | 휴식/준비 | **0회** | 🟢 정상 |
| **5. 풋워크 (Footwork)** | `57~70s` | 휴식/준비 | **9회** | ⚠️ 9회 오검출 |
| **⏸ 숨고르기 (Rest 4)** | `70~75s` | 휴식/준비 | **1회** | ⚠️ 1회 오검출 |
| **6. 실전 콤보 (Combos)** | `75~85s` | 동작 | **11회** | 🟢 정상 |
...
1. **비동작 구간(풋워크/휴식) 과검출 방지**:
   * 현재 풋워크/휴식 구간에서 총 **11회**의 오검출이 발생했습니다.
