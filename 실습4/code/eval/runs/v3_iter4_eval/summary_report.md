...
| :--- | :---: | :---: | :---: |
| **F1-Score (종합)** | **0.3704** | $\ge 0.850$ | 🟡 REVIEW |
| **Precision (정밀도)** | **0.4000** (10/25) | $\ge 0.800$ | 🟡 REVIEW |
| **Recall (재현율)** | **0.3448** (10/29) | $\ge 0.850$ | 🟡 REVIEW |
| **종류 분류 정확도 (Kind Acc)** | **40.0%** | $\ge 90.0\%$ | 🟡 REVIEW |
| **비동작/풋워크 오검출 (Non-Action FP)** | **5회** | $\le 3회$ | 🔴 과검출 발생 |
| **평균 타격 지연 시간 (Timing MAE)** | **206.7 ms** | $\le 50.0 ms$ | 🟡 REVIEW |
...
| **2. 직선 펀치 (Straight)** | `06~19s` | 동작 | **4회** | 🟢 정상 |
...
1. **비동작 구간(풋워크/휴식) 과검출 방지**:
   * 현재 풋워크/휴식 구간에서 총 **5회**의 오검출이 발생했습니다.
