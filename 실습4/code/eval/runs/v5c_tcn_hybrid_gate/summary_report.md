...
**생성 일시**: 2026-08-27 12:49:49  
**평가 대상**: `C:\DS_Team2\project-4\iter4\eval\video\benchmark.mp4`  
**허용 오차 윈도우**: `±400ms`
...
| :--- | :---: | :---: | :---: |
| **F1-Score (종합)** | **0.5763** | $\ge 0.850$ | 🟡 REVIEW |
| **Precision (정밀도)** | **0.5667** (17/30) | $\ge 0.800$ | 🟡 REVIEW |
| **Recall (재현율)** | **0.5862** (17/29) | $\ge 0.850$ | 🟡 REVIEW |
| **종류 분류 정확도 (Kind Acc)** | **58.8%** | $\ge 90.0\%$ | 🟡 REVIEW |
| **비동작/풋워크 오검출 (Non-Action FP)** | **0회** | $\le 3회$ | 🟢 PASS |
| **평균 타격 지연 시간 (Timing MAE)** | **155.2 ms** | $\le 50.0 ms$ | 🟡 REVIEW |
...
| **2. 직선 펀치 (Straight)** | `06~19s` | 동작 | **9회** | 🟢 정상 |
| **⏸ 숨고르기 (Rest 1)** | `19~23s` | 휴식/준비 | **0회** | 🟢 정상 |
| **3. 훅 펀치 (Hook)** | `23~35s` | 동작 | **8회** | 🟢 정상 |
| **⏸ 숨고르기 (Rest 2)** | `35~40s` | 휴식/준비 | **0회** | 🟢 정상 |
| **4. 어퍼컷 (Uppercut)** | `40~52s` | 동작 | **6회** | 🟢 정상 |
| **⏸ 숨고르기 (Rest 3)** | `52~57s` | 휴식/준비 | **0회** | 🟢 정상 |
| **5. 풋워크 (Footwork)** | `57~70s` | 휴식/준비 | **0회** | 🟢 정상 |
| **⏸ 숨고르기 (Rest 4)** | `70~75s` | 휴식/준비 | **0회** | 🟢 정상 |
| **6. 실전 콤보 (Combos)** | `75~85s` | 동작 | **7회** | 🟢 정상 |
...
```text
  ...
  • HOOK->STRAIGHT           : 2회
  • STRAIGHT->HOOK           : 1회
  • STRAIGHT->STRAIGHT       : 8회
  • STRAIGHT->UPPERCUT       : 2회
  • UPPERCUT->HOOK           : 2회
  • UPPERCUT->UPPERCUT       : 1회
```
...
1. **비동작 구간(풋워크/휴식) 과검출 방지**:
   * 현재 풋워크/휴식 구간에서 총 **0회**의 오검출이 발생했습니다.
