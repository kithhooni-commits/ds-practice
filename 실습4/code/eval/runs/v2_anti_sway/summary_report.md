...
| :--- | :---: | :---: | :---: |
| **F1-Score (종합)** | **0.3514** | $\ge 0.850$ | 🟡 REVIEW |
| **Precision (정밀도)** | **0.2889** (13/45) | $\ge 0.800$ | 🟡 REVIEW |
| **Recall (재현율)** | **0.4483** (13/29) | $\ge 0.850$ | 🟡 REVIEW |
| **종류 분류 정확도 (Kind Acc)** | **23.1%** | $\ge 90.0\%$ | 🟡 REVIEW |
| **비동작/풋워크 오검출 (Non-Action FP)** | **6회** | $\le 3회$ | 🔴 과검출 발생 |
| **평균 타격 지연 시간 (Timing MAE)** | **115.6 ms** | $\le 50.0 ms$ | 🟡 REVIEW |
...
1. **비동작 구간(풋워크/휴식) 과검출 방지**:
   * 현재 풋워크/휴식 구간에서 총 **6회**의 오검출이 발생했습니다.
