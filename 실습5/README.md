...
| | 학습 데이터 | test 입력 사용 | PSNR | SSIM |
|---|---|---|---|---|
| A | `test_noise_only` 100장 | **사용** (test-time adaptation) | 30.95 | 0.8992 |
| **B** | `train/` clean 에서 합성한 noisy 7,268장 | **미사용** | **31.38** | **0.9056** |
...
> **정정.** 처음에는 B 를 돌려 보고 "데이터를 73배 늘려도 3 dB 나쁘다, 평가 대상에
> 적응하는 쪽이 유리하다"고 결론지었다. **그 결론은 틀렸다.** 그 실행은 fp16 + lr 3e-4
> 였고, DRUNet 을 NaN 으로 발산시킨 것과 같은 조합이었다. loss 가 진동하며 오르는데도
> 현상으로 해석하고 10,000 step 에서 중단했다. bf16 + lr 1e-4 로 다시 돌리니 정상
> 학습됐다. 실행 하나의 실패를 성질로 착각한 것이다.

주의: A 는 DnCNN, B 는 DRUNet 이라 데이터 축과 구조 축이 섞여 있다.
같은 조건(DnCNN)으로 B 를 돌린 결과를 추가해 분리할 예정이다.
...
- [ ] 같은 조건(DnCNN)으로 B 재실행 — 데이터 축과 구조 축 분리
