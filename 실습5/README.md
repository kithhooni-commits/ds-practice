...
```
PSNR_total 36.50    SSIM_total 0.9580
```

배포 기준선(DnCNN 10 epoch) 30.51 / 0.8950 대비 **+5.99 dB / +0.0630**.
A100 에서 60 epoch, **DRUNet** + 8× self-ensemble.
...
| **DRUNet 60ep + SE (제출)** | 7,268장 | **36.50** | **0.9580** |
...
|---|---|---|---|---|
| DnCNN | 0.59M | 35px | 34.56 | 0.9489 |
| **DRUNet** | 32.6M | ~180px | **36.50** | **0.9580** |

같은 데이터·같은 레시피·같은 60 epoch 인데 **+1.94 dB**. 노이즈별로 보면 차이가 어디서
나는지 분명하다.

| noise | DnCNN | DRUNet | Δ |
|---|---|---|---|
...
- [x] DRUNet 학습 (A100) — 36.50 / 0.9580 로 제출값 확정
