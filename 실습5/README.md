...
| **label-free (test 100장만)** | **0장** | **30.95** | **0.8992** |
...
```
label-free (test 100장만, clean 0장)   PSNR 30.95   SSIM 0.8992
배포 기준선 (clean 7,268장 supervised)  PSNR 30.51   SSIM 0.8950
```

**정답을 한 장도 안 쓰고 배포 기준선을 넘었다** (+0.44 dB / +0.0042). clean 7,268장으로
지도학습한 모델보다 낫다는 뜻이다. median 3×3 필터(26.68)는 4.3 dB 앞선다.

검증 loss 가 16,000 step 까지 계속 내려가고 있었으므로 아직 수렴 전이다. 더 돌릴 여지가 있다.
...
- [x] label-free 를 순정 DnCNN 으로 재실행 — 30.95 / 0.8992, 배포 기준선 초과
