...
### 딥러닝으로도 TKD 를 넘을 수 있다

과제가 U-Net 을 주고 긴 학습 시간을 준 것은 **딥러닝으로 풀어 보라**는 뜻이다.
해석적 역산(109.86)은 몇 초면 끝나므로 그것만 내는 건 과제가 원한 답이 아니다.

| 방법 | 커널 | PSNR | SSIM |
|---|---|---|---|
| 배포 U-Net 30 epoch | 안 씀 | 25.59 | 0.8779 |
| U-Net f16 (크롭 128) | 안 씀 | 26.71 | 0.9322 |
| U-Net f48 (크롭 128) | 안 씀 | 28.59 | 0.9536 |
| DRUNet f64 (크롭 128) | 안 씀 | 25.53 | 0.9406 |
| **Spectral + U-Net** | **안 씀** | **37.33** | **0.9781** |
| **DC-Net (tau 0.05)** | 사용 | **42.93** | **0.9895** |
| TKD t=0.15 (넘어야 할 선) | 사용 | 31.19 | 0.9337 |

**순수 end-to-end 는 넘지 못한다.** 용량을 3배 키워도(f16 → f48) 28.59 에 그치고,
가장 큰 DRUNet 이 오히려 가장 낮다. 크롭 128 로 학습한 탓이 크다 — deconvolution 은
전역 연산이라 조각만 보면 복원에 필요한 정보가 조각 밖에 있다. 1일차에서 크롭이
증강으로 작동한 것과 정반대다.

**구조를 바꾸면 넘는다.**

- **Spectral + U-Net** — 첫 층을 주파수 곱셈(65,536개 학습 파라미터)으로 두고
  뒤에 U-Net 을 붙인다. 전역 역산은 주파수 층이, 국소 정리는 U-Net 이 맡는다.
  **커널을 쓰지 않는다** — 역필터를 데이터에서 배운다.
- **DC-Net** — |D| > tau 인 주파수는 G/D 로 정확히 복원하고 네트워크가 못 건드리게
  한 뒤, 사라진 null cone 만 U-Net 이 채운다. 문제가 "복원"에서 "빠진 줄무늬 메우기"로
  바뀌어 CNN 이 잘하는 일이 된다.

### 학습률이 실패의 원인이었다

주파수 곱셈을 경사하강으로 학습할 때 26 dB 에서 멈춘 것을 처음에는 "조건수 때문에
경사하강으로는 불가능"이라고 결론냈다. **틀렸다.**

| 주파수 층 lr | val PSNR | 학습된 최대 이득 |
|---|---|---|
| 0.01 | 32.73 | 12.5 |
| 0.10 | 41.65 | 96.3 |
| 0.50 | 32.36 | 220 (저주파가 불안정) |
| 정답 | 111.03 | 44,097 |

이득이 44,000 까지 가야 하는데 나머지 층과 같은 lr 을 쓰면 도달할 수가 없다.
gradient 가 |D| 에 비례해 작은 건 맞지만, **작은 gradient 는 큰 학습률로 보상하면 된다.**
파라미터 그룹을 나눠 주파수 층에만 큰 lr 을 준다 (`--lr-spectral`).

### tau 를 낮출 여지가 남아 있다

DC-Net 의 tau 는 "네트워크에게 얼마나 어려운 일을 시킬 것인가"만 정한다.

| tau | 가려지는 주파수 | 네트워크가 아무것도 안 할 때 | 완벽할 때 |
|---|---|---|---|
| 0.005 | 0.81% | 42.74 | 118.98 |
| 0.02 | 3.19% | 38.22 | 119.16 |
| 0.05 | 7.98% | 32.90 | 119.19 |

노이즈가 없으므로 tau 를 낮출 때 잃을 것이 없다. 0.05 는 있지도 않은 노이즈에 대비한
과보수였고, 0.005 면 네트워크가 아무 일도 안 해도 42.74 다.
