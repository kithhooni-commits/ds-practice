...
**오라클 위너(장마다 참 스펙트럼을 넣은 최고의 선형 필터)조차 21.95 dB 에서 끝난다**
(`oracle_linear.py`). 학습 없는 조합(1일차 디노이저 → Wiener)이 21.06 으로 거기
**0.89 dB 까지 닿아 있다** — 선형으로 짜낼 것은 다 짜냈다는 뜻이다. 그 위 **7.75 dB 는
순전히 학습된 사전지식의 몫**이고, 그래서 3일차는 도구를 조립하는 문제가 아니라
**역산이 포기한 원뿔 안을 이미지 사전지식으로 메우는 문제**다.

처음에는 여러 장 평균 스펙트럼으로 재서 19.00 을 천장이라 적었다. 장마다 따로 맞추는
쪽이 상한으로 맞다. (주파수마다 정답에 맞춘 복소수를 곱하면 119 dB 가 나오지만 자유도가
화소 수와 같아 정답을 베끼는 것이라 상한이 아니다.)
