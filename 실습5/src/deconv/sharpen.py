...
과평활은 실재한다. 그런데 언샤프로는 되돌릴 수 없다.
    ...
    복원의 국소 std 0.07904 · 정답 0.08277 · 비율 0.955   <- 뭉갰다 (val 25장)

    후처리 전            19.71 dB / 0.4762
    amount 0.10 s 0.8    19.52 / 0.4682
    amount 0.30 s 0.8    19.12 / 0.4523
    amount 1.00 s 0.8    17.66 / 0.3984   <- 세게 걸수록 단조롭게 나빠진다

    -> val 에서 고르게 하면 amount=0.00 을 고른다. 즉 **스스로 꺼진다.**
...
튜닝이 val 에서 amount=0 을 고르므로 `--sharpen` 을 켜 두어도 손해는 없다 —
혹시 다른 모델에서는 듣는지 확인하는 값싼 점검으로 쓸 수 있다.
"""
