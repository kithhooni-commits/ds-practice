...
def build(prs, name: str, M: dict, R: dict) -> None:
    ...
    rows = [("특징", "하위 1/3 → 상위 1/3", "상관계수", "해석"),
            ("에지 밀도", "−7.78 dB", "−0.442", "구조가 복잡할수록 나쁘다. 가장 강한 예측 변수"),
            ("대비 (std)", "−4.34 dB", "−0.279", "σ 는 이미지와 무관하게 뽑히므로 대비가 클수록 불리"),
            ("동적범위", "−3.47 dB", "−0.234", "위와 같은 이유"),
            ("무늬 조밀도", "−3.11 dB", "−0.179", "고주파일수록 |D| 가 작아 역산이 어렵다")]
    ...
    footer(sl, "가장 못 살린 10장 — rician 8 · uniform 2 · gaussian 0 · s&p 0   |   "
               "σ 0.120 (전체 0.074) · 에지 0.083 (0.050).  약점은 '강한 rician + 복잡한 구조' 다")
