...
def build(prs, name: str, M: dict, R: dict) -> None:
    ...
    for tag, head, body in [
        ...
        ("한계", "선형 방법의 천장은 21.95 dB",
         "정답을 알고 만든 최고의 선형 필터조차 그렇다. 학습 없는 조합이 21.06 으로 거기 거의 닿아 있고, "
         "그 위 7.75 dB 가 학습된 사전지식의 몫이다."),
    ...
    # ---------------------------------------------------------- 4. 선형의 천장
    sl = blank(prs); bg(sl)
    title(sl, "선형 방법의 천장은 21.95 dB", "정답을 알고 만든 최고의 선형 필터조차 그렇다",
          eyebrow="왜 딥러닝인가")
    rows = [("방법", "PSNR", "성격"),
            ("Wiener 단독", "13.79", "선형"),
            ("median → Wiener", "19.20", "거의 선형"),
            ("1일차 디노이저 → Wiener", "21.06", "학습 없는 조합 중 최고"),
            ("오라클 위너 (장마다 참 SNR 을 안다고 가정)", "21.95", "선형의 천장. 만들 수 없다"),
    ...
    text(sl, Inches(1.2), Inches(5.0), Inches(11), Inches(1.2),
         [("학습 없는 조합(21.06)이 선형 오라클(21.95)에 0.89 dB 까지 닿아 있다. 짜낼 것은 다 짜냈다.",
           {"bold": True, "color": INK}),
          ("그 위 7.75 dB 는 순전히 학습된 사전지식의 몫이다.", {}),
          ("3일차는 도구를 조립하는 문제가 아니라, 역산이 포기한 원뿔 안을 이미지 사전지식으로 메우는 문제다.", {})],
         size=14, color=INK2)
    ...
    rows = [("특징", "하위 1/3 → 상위 1/3", "상관계수", "해석"),
            ...
            ("대비 (std)", "−4.33 dB", "−0.299", "노이즈 σ 는 이미지와 무관하게 뽑히므로 대비가 클수록 불리"),
    ...
    table(sl, Inches(0.9), Inches(4.6), Inches(11.5), Inches(1.6), rows,
          col_w=[2.0, 2.2, 1.5, 5.8])
    footer(sl, "가장 못 살린 10장 — rician 8장 · uniform 2장 · gaussian 0 · s&p 0   |   "
